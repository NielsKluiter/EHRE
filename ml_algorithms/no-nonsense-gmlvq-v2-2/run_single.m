...
prdiff = abs(tpr-(1-fpr));
[test,best] = min(prdiff);
bias = log(5001/best-1)*2;

lambda=om'*om;   % actual relevance matrix
% define structures corresponding to the trained system and training curves
gmlvq_system =    struct('protos',prot,'lambda',lambda,'plbl',plbl,...
                         'mean_features',mf,'std_features',st,'bias',bias); 
